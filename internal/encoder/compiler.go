...
func compileToGetCodeSetSlowPath(typeptr uintptr, customTag string) (*OpcodeSet, error) {
...
func (c *Compiler) compile(typeptr uintptr, customTag string) (*OpcodeSet, error) {
	// noescape trick for header.typ ( reflect.*rtype )
	typ := *(**runtime.Type)(unsafe.Pointer(&typeptr))
	code, err := c.typeToCode(typ, customTag)
	if err != nil {
		return nil, err
	}
...
func (c *Compiler) typeToCode(typ *runtime.Type, customTag string) (Code, error) {
	switch {
	...
	case reflect.Slice:
		...
		return c.sliceCode(typ, customTag)
	case reflect.Map:
		if isPtr {
			return c.ptrCode(runtime.PtrTo(typ), customTag)
		}
		return c.mapCode(typ, customTag)
	case reflect.Struct:
		return c.structCode(typ, isPtr, customTag)
	case reflect.Int:
	...
	default:
		...
		return c.typeToCodeWithPtr(typ, isPtr, customTag)
	}
}

func (c *Compiler) typeToCodeWithPtr(typ *runtime.Type, isPtr bool, customTag string) (Code, error) {
	switch {
	case c.implementsMarshalJSON(typ):
		return c.marshalJSONCode(typ)
	case c.implementsMarshalText(typ):
	...
	switch typ.Kind() {
	case reflect.Ptr:
		return c.ptrCode(typ, customTag)
	case reflect.Slice:
		elem := typ.Elem()
		if elem.Kind() == reflect.Uint8 {
			p := runtime.PtrTo(elem)
		...
		return c.sliceCode(typ, customTag)
	case reflect.Array:
		return c.arrayCode(typ, customTag)
	case reflect.Map:
		return c.mapCode(typ, customTag)
	case reflect.Struct:
		return c.structCode(typ, isPtr, customTag)
...
func (c *Compiler) ptrCode(typ *runtime.Type, customTag string) (*PtrCode, error) {
	code, err := c.typeToCodeWithPtr(typ.Elem(), true, customTag)
	if err != nil {
		return nil, err
	}
	ptr, ok := code.(*PtrCode)
...
func (c *Compiler) sliceCode(typ *runtime.Type, customTag string) (*SliceCode, error) {
	elem := typ.Elem()
	code, err := c.listElemCode(elem, customTag)
	if err != nil {
		return nil, err
	}
...
func (c *Compiler) arrayCode(typ *runtime.Type, customTag string) (*ArrayCode, error) {
	elem := typ.Elem()
	code, err := c.listElemCode(elem, customTag)
	if err != nil {
		return nil, err
...
func (c *Compiler) mapCode(typ *runtime.Type, customTag string) (*MapCode, error) {
	keyCode, err := c.mapKeyCode(typ.Key(), customTag)
	if err != nil {
		return nil, err
	}
	valueCode, err := c.mapValueCode(typ.Elem(), customTag)
	if err != nil {
		return nil, err
	}
...
func (c *Compiler) listElemCode(typ *runtime.Type, customTag string) (Code, error) {
	switch {
	...
	default:
		...
		code, err := c.typeToCodeWithPtr(typ, true, customTag)
		if err != nil {
			return nil, err
		}
		ptr, ok := code.(*PtrCode)
...
func (c *Compiler) mapKeyCode(typ *runtime.Type, customTag string) (Code, error) {
	switch {
	case c.implementsMarshalJSON(typ):
		return c.marshalJSONCode(typ)
	case c.implementsMarshalText(typ):
...
func (c *Compiler) mapValueCode(typ *runtime.Type, customTag string) (Code, error) {
	switch typ.Kind() {
	case reflect.Map:
		return c.ptrCode(runtime.PtrTo(typ), customTag)
	default:
		code, err := c.typeToCodeWithPtr(typ, false, customTag)
		if err != nil {
			return nil, err
		}
		ptr, ok := code.(*PtrCode)
...
func (c *Compiler) structCode(typ *runtime.Type, isPtr bool, customTag string) (*StructCode, error) {
	...
	tags := c.typeToStructTags(typ, customTag)
	fields := []*StructFieldCode{}
	for i, tag := range tags {
		isOnlyOneFirstField := i == 0 && fieldNum == 1
		field, err := c.structFieldCode(code, tag, isPtr, isOnlyOneFirstField, customTag)
		if err != nil {
			return nil, err
		}
...
func (c *Compiler) structFieldCode(structCode *StructCode, tag *runtime.StructTag, isPtr, isOnlyOneFirstField bool, customTag string) (*StructFieldCode, error) {
	field := tag.Field
	...
	case isPtr && c.isPtrMarshalTextType(fieldType):
		...
		fieldCode.isNilCheck = false
	default:
		code, err := c.typeToCodeWithPtr(fieldType, isPtr, customTag)
		if err != nil {
			return nil, err
		}
		switch code.Kind() {
...
func (c *Compiler) typeToStructTags(typ *runtime.Type, customTag string) runtime.StructTags {
